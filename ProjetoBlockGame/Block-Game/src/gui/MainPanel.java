 ...
 */
public class MainPanel extends javax.swing.JPanel implements MouseListener, KeyEventPostProcessor {
    ...
    public void paintComponent(Graphics g) {
        ...
        Dimension size = getSize();
        g2d.drawImage(core.getGameController().getCurrentFactory().createBackground().carregar(), 0, 0, null);
        g2d.drawImage(core.getGameController().getCurrentFactory().createChao().carregar(), size.width/2-450, size.height/2-10+260, null);

        for (Rectangle rect : m_bodyRect.values()) {
            if (rect != m_player) {
                // Block
                try {
                    TexturePaint texturePaint = new TexturePaint(ImageIO.read(core.getGameController().getCurrentFactory().createBlocos().carregar()), rect);
